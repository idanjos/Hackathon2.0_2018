...
global thread1
ip = "127.0.0.1:5000" #Node's location
...
def receive():
	s=socket(AF_INET, SOCK_DGRAM)
	s.setsockopt(SOL_SOCKET, SO_BROADCAST, 1)
	s.bind(('',12346)) # binding to port to listen
	while(1):
		m=s.recvfrom(1024)
...
thread1 = threading.Thread(target = tcp_server,args=('127.0.0.1',5000,file)) # Tcp Server to receive file
thread1.start()
...
while(1):
	...
	if thread1.isAlive() and stopads < 2:
		s.sendto(msg.encode(),('255.255.255.255',12345)) # Broadcast request
		time.sleep(5)
		stopads+=1
	else:
		if stopads == 2:
